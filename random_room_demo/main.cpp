...
// Camera State
struct FreeCamera {
    glm::vec3 position{0.0f, 20.0f, 60.0f};
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(1280, 720, "JOLTrl - Random Policy Room", nullptr, nullptr);
    if (!window) return 1;
    ...
    std::vector<float> a1(ACTIONS_PER_ROBOT), a2(ACTIONS_PER_ROBOT);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (!paused) {
            for (int i = 0; i < ACTIONS_PER_ROBOT; ++i) { a1[i] = dis(gen); a2[i] = dis(gen); }
            loader.ApplyActions(r1, a1.data(), &physicsCore.GetPhysicsSystem());
            loader.ApplyActions(r2, a2.data(), &physicsCore.GetPhysicsSystem());
        ...
        ui.BeginFrame();
        ImGui::Begin("Random Policy Room");
        ImGui::Text("2 Robots | Random Actions");
        ...
        if (ImGui::CollapsingHeader("Robot Power (Incremental)")) {
            // Apply to both robots
            ImGui::SliderFloat("Rotation Power", &r1.actionScale.rotationScale, 0.0f, 500.0f);
            ImGui::SliderFloat("Stabbing Power", &r1.actionScale.slideScale, 0.0f, 2000.0f);
            
            // Sync r2 to r1 for consistent testing
        ...
        if (ImGui::CollapsingHeader("Physics Tuner")) {
            bool changed = false;
            int v = settings.mNumVelocitySteps, p = settings.mNumPositionSteps;
