...
bool PhysicsCore::Init(uint32_t numParallelEnvs)
{
    ...
    const uint32_t maxBodies = std::max<uint32_t>(2048, mNumEnvs * bodiesPerEnv + 512);
    const uint32_t numBodyMutexes = std::max<uint32_t>(1, mNumEnvs / 2); // Mutex per ~2 envs
    const uint32_t maxBodyPairs = std::min<uint32_t>(131072, maxBodies * 8); // Increase to 128K
