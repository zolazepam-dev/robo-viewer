 ...
 * 
 * Provides the same interface as ODE2VAEDynamics but uses RFF internally.
 * This allows seamless switching between ODE and RFF dynamics.
 */
class alignas(32) RFFDynamicsWrapper
{
public:
    RFFDynamicsWrapper() = default;
    RFFDynamicsWrapper(const RFFDynamicsWrapper& other) = default;
    RFFDynamicsWrapper& operator=(const RFFDynamicsWrapper& other) = default;
    ...
    // Compatibility methods for ODE2VAEDynamics interface
...
public:
    ...
    // Use RFF dynamics wrapper for RFF-based dynamics
    RFFDynamicsWrapper& GetDynamics() { return mDynamics; }
    const RFFDynamicsWrapper& GetDynamics() const { return mDynamics; }
...
private:
    ...
    // Batch buffers for RFF dynamics (reused across calls)
    AlignedVector32<float> mCombinedInputBatch;
    AlignedVector32<float> mAccelOutBatch;
};
