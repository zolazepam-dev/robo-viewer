...
// ============================================================================
// HIGH-PERFORMANCE TD3Trainer - Fully Vectorized & Batched
// ============================================================================
    ...
    , mOpponentPool(MAX_POOL_SIZE)
{
    ...
    // Pre-allocate ALL buffers with 32-byte alignment for AVX2
...
// ============================================================================
// TRAINING - Main entry point (called every N steps)
// ============================================================================
...
void TD3Trainer::Train(ReplayBuffer& buffer)
{
    ...
    // Update actor (delayed - every policyDelay steps)
    ...
    // Snapshot opponent for self-play
...
// ============================================================================
// CRITIC UPDATE - Fully batched, vectorized
// ============================================================================
...
// ============================================================================
// CRITIC UPDATE (Vector Rewards) - Fully batched
// ============================================================================
...
void TD3Trainer::UpdateCriticWithVectorRewards(ReplayBuffer& buffer)
{
    ...
    // Build critic input with latents - BATCH
    AlignedVector32<float> zPos(latentDim);
    AlignedVector32<float> zVel(latentDim);
    ...
    // Update critics (same as UpdateCritic)
    for (int i = 0; i < batchSize; ++i)
    {
        int envIdx = 0;
        mModel.GetLatentMemory().GetLatentStates(zPos.data(), zVel.data(), envIdx);
    ...
    for (size_t w = 0; w < weights1.size() && w < mGrads.size(); w += 128)
    {
        ...
        if (newLoss > originalLoss) weights1[w] -= 2.0f * criticLR;
    }
    critic1.SetAllWeights(weights1);
}

// ============================================================================
// ACTOR UPDATE - Fully batched, vectorized, sampled weights
// ============================================================================
