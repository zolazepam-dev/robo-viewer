...
void OverlayUIRefactored::Init(GLFWwindow* window)
{
    ...
    DrawCyberpunkStyle();
    LoadSettings(); // This now calls ConfigManager and syncs to UI state
}
...
void OverlayUIRefactored::DrawTrainingTab()
{
    ...
    if (ImGui::Button(mPaused ? "RESUME" : "PAUSE")) mPaused = !mPaused;
    ImGui::SameLine();
    if (ImGui::Button("SAVE CONFIG")) SaveSettings();
...
void OverlayUIRefactored::DrawPhysicsTab()
{
    ...
    ImGui::SliderFloat("Time Scale", &mTimeScale, 0.1f, 4.0f);
}

void OverlayUIRefactored::DrawRobotsTab()
{
    if (ImGui::Combo("Type", &mRobotSelection.selectedRobotIndex, "Internal\0Shell\0Wedge\0Drum\0\0")) {
        // Simple switch for robot params
    }
    if (ImGui::Button("LOAD CONFIG")) mRobotSelection.loadConfigRequested = true;
}

void OverlayUIRefactored::DrawGraphicsTab()
{
    ImGui::Checkbox("Collision Shapes", &mGraphics.showCollisionShapes);
    ImGui::SliderFloat("Cam Dist", &mGraphics.cameraDistance, 5.0f, 50.0f);
}
...
// Redefinitions removed (already in header)
