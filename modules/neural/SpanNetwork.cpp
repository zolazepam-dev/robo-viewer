...
{
    size_t total = GetNumWeights();
    std::vector<float> weights(total);
    float* ptr = weights.data();
    for (const auto& layer : mLayers)
    {
        const auto& cp = layer.GetControlPoints();
        size_t n = cp.size();
        std::memcpy(ptr, cp.data(), n * sizeof(float));
        ptr += n;
    }
    return weights;
}
...
{
    const float* ptr = weights.data();
    for (auto& layer : mLayers)
    {
        auto& cp = layer.GetControlPoints();
        size_t n = cp.size();
        std::memcpy(cp.data(), ptr, n * sizeof(float));
        ptr += n;
    }
}
...
{
    // OPTIMIZATION: In-place soft update without vector copies
    for (size_t i = 0; i < mLayers.size(); ++i)
    {
        auto& myCP = mLayers[i].GetControlPoints();
        const auto& otherCP = other.mLayers[i].GetControlPoints();
        SoftUpdate_AVX2(myCP.data(), otherCP.data(), myCP.size(), tau);
    }
}
