...
void SoftUpdate_AVX2(float* target, const float* source, size_t size, float tau)
{
    const size_t simdWidth = 8;
    const size_t simdEnd = size - (size % simdWidth);
    const __m256 tau_vec = _mm256_set1_ps(tau);
    const __m256 one_minus_tau_vec = _mm256_set1_ps(1.0f - tau);
    ...
    for (; i < simdEnd; i += simdWidth)
    {
        ...
        __m256 result = _mm256_mul_ps(one_minus_tau_vec, t);
        result = _mm256_fmadd_ps(tau_vec, s, result);
    ...
    for (; i < size; ++i)
    {
        target[i] = one_minus_tau * target[i] + tau * source[i];
    }
}
