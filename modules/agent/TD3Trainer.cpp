...
void TD3Trainer::Load(const std::string& path)
{
    ...
    {
        std::cerr << "[TD3Trainer] ERROR: Dimension mismatch!" << std::endl;
        std::cerr << "[TD3Trainer]   Expected: stateDim=" << mStateDim << ", actionDim=" << mActionDim << std::endl;
        std::cerr << "[TD3Trainer]   Got:      stateDim=" << stateDim << ", actionDim=" << actionDim << std::endl;
        std::cerr << "[TD3Trainer]   This model was trained with a different robot architecture." << std::endl;
        return;
    }
